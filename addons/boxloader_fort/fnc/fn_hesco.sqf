...
_arr = [];
if (_big==0) then {
	_arr=[["Build HESCO (1)","Land_Boxloader_Hesco_1"],["Build HESCO (2)","Land_Boxloader_Hesco_2"],["Build HESCO (3)","Land_Boxloader_Hesco_3"],["Build HESCO (5)","Land_Boxloader_Hesco_5"]];
};
if (_big==1) then {
...
},[],0.5,false,true,"","isNull attachedTo _target"];
{
	...
	},[_x select 1],0,false,true,"",[false,false] call boxloader_fort_fnc_buildcondition];
