...
},[],0.5,false,true,"","(count ((position _target) nearObjects ['B_APC_Tracked_01_CRV_F',20])>0) && (isNull attachedTo _target)"];
...
},[],0.5,false,true,"","isNull attachedTo _target"];
{
	...
	},[_x select 1],0,false,true,"","(isNull attachedTo _target) && (_target getVariable ['BuildMenu',false]) && (count ((position _target) nearObjects ['B_APC_Tracked_01_CRV_F',20])>0)"];

} forEach [
	["Build HESCO (1)","Land_Boxloader_Hesco_1"],
	["Build HESCO (3)","Land_Boxloader_Hesco_3"],
	["Build HESCO (5)","Land_Boxloader_Hesco_5"]
];
true 