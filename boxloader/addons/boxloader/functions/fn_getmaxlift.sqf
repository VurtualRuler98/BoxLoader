...
_height = (abs (_bedheight - ((getPosATL _box select 2) min (getPosASL _box select 2))));
if ((_tgt getVariable ["boxloader_crane_weight",0]>_mass) && (_tgt getVariable ["boxloader_crane_height",0]>_height)) exitWith {true};
if (_height < 2.1 && _mass < boxloader_maxload_overhead) exitWith {true};
if (_height < 1.3 && _mass < boxloader_maxload_lift) exitWith {true};
if (_height < 0.4 && _mass < boxloader_maxload_push) exitWith {true};
false
