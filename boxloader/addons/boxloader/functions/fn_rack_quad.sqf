...
_veh addAction ["Use rear cargo rack",{[(_this select 0),(_this select 1),true] remoteExecCall ["boxloader_fnc_rack_quad_switch",(_this select 0)]},[],0,false,true,"","(vehicle _this == _this) && (isNull (fullCrew [_target,'cargo',true] select 0 select 0)) && ((_target getVariable ['boxloader_rack1',objNull]) getVariable ['boxloader_dontload',false])"];
_veh addAction ["Use rear passenger seat",{[(_this select 0),(_this select 1),false] remoteExecCall ["boxloader_fnc_rack_quad_switch",(_this select 0)]},[],0,false,true,"","(vehicle _this == _this) && (isNull (fullCrew [_target,'cargo',true] select 0 select 0)) && !((_target getVariable ['boxloader_rack1',objNull]) getVariable ['boxloader_dontload',true]) && ((count getVehicleCargo (_target getVariable ['boxloader_rack1',objNull]))==0)"];
...
_rack0 = "Land_Boxloader_mem_quad_front" createVehicle position _veh;
_rack1 = "Land_Boxloader_mem_quad" createVehicle position _veh;
