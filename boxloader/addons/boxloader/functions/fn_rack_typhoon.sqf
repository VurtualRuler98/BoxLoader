...
if (!(_veh isKindOf "O_Truck_03_covered_F") && !(_veh isKindOf "O_Truck_03_transport_F")) exitWith {["That's not an acceptable vehicle class!"] call bis_fnc_error; false};
if (_veh getVariable ["boxloader_rackinit",false]) exitWith {false};
_veh setVariable ["boxloader_rackinit",true,true];
_veh addAction ["Use rear cargo bed",{[(_this select 0),(_this select 1),true] remoteExecCall ["boxloader_fnc_rack_typhoon_switch",(_this select 0)]},[],0,false,true,"","(vehicle _this == _this) && (({!(isNull (_x select 0))} count fullCrew [_target, 'turret',true])==0) && (({!(isNull (_x select 0)) && (_x select 2)>0} count fullCrew [_target, 'cargo',true])==0) && ((_target getVariable ['boxloader_rack0',objNull]) getVariable ['boxloader_dontload',false])"];
_veh addAction ["Use rear cargo seats",{[(_this select 0),(_this select 1),false] remoteExecCall ["boxloader_fnc_rack_typhoon_switch",(_this select 0)]},[],0,false,true,"","(vehicle _this == _this) && !((_target getVariable ['boxloader_rack0',objNull]) getVariable ['boxloader_dontload',true]) && ((count getVehicleCargo (_target getVariable ['boxloader_rack0',objNull]))==0)"];
...
_rack0 = "Land_Boxloader_mem_typhoon" createVehicle position _veh;
_rack0 attachTo [_veh,[0,-2.5,-0.38]];
